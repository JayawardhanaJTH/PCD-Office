    ...
    if (isset($_FILES["event_image"])) {
        ...
        if (in_array($event_image_ext, $extensions) === false) {
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                //send event details to registered users
                $sql_people = "SELECT firstname,lastname,email FROM people";
                ...
                if (mysqli_num_rows($result_people) > 0) {

                    while ($row = mysqli_fetch_row($result_people)) {

                        $to = $row[2];
                        $mailSubject =  "PCD Office New Event";
                        $emailBody = "Dear <b>" . $row[0] . " " . $row[1] . "</b> <br>
               New event is published by PCD Office. <br>
                <b>Event name :</b> $event_name <br>
                <b>Event date :</b> $event_date <br>Thank You !";
